...
createCar maximoP maximoG = Info (Car 0 maximoP 0 maximoG 0) (Op "create" True)

-- enche o tanque passando a qtd de gas. Retorna falso apenas se o tanque já estiver completamente cheio.
fuel :: Int -> Info -> Info
fuel gasosa (Info (Car pass maxPass gas maxGas km) _) =
    if gas==maxGas then Info (Car pass maxPass gas maxGas km) (Op "fuel" False)  
    else if gasosa+gas > maxGas then Info (Car pass maxPass maxGas maxGas km) (Op "fuel" True)
    else Info (Car pass maxPass (gas+gasosa) maxGas km) (Op "fuel" True)

-- Faz entrar uma pessoa no carro. Retorna false se já estiver lotado.
embark :: Info -> Info 
embark (Info(Car pass maxPass gas maxGas km)_) = 
    if pass==maxPass then Info (Car pass maxPass gas maxGas km) (Op "embark" False)
    else Info (Car (pass+1) maxPass gas maxGas km) (Op "embark" True)

-- Retira uma pessoa do carro, retorna false se não tiver ninguém no carro
disembark :: Info -> Info
disembark (Info(Car pass maxPass gas maxGas km)_) = 
    if pass==0 then Info (Car pass maxPass gas maxGas km) (Op "disembark" False)
    else Info (Car (pass-1) maxPass gas maxGas km) (Op "disembark" True)
...
drive :: Int -> Info -> Info 
drive kilo (Info(Car pass maxPass gas maxGas km)_) = 
    if (pass==0) || (gas<kilo) then Info (Car pass maxPass gas maxGas km) (Op "drive" False)
    else Info (Car pass maxPass (gas-kilo) maxGas (km+kilo)) (Op "drive" True)
